    ...
    <Wrapper>
      ...
      <Slider {...sliderSettings}>
        {homeCmsData?.homebanner?.map((banner, key) => {
          return (
            <div>
              <Typography className={styles.carousel_caption}>{banner?.banner_title}</Typography>
              <div
                style={{paddingLeft: "35vw"}}
                className={styles.carousel_caption2}
                dangerouslySetInnerHTML={{ __html: banner?.banner_description }}
              />
              <img src={banner?.banner_image_path} alt="" style={{position: 'relative'}}/>
              <CustomButtonPrimary
                  type="button"
                  variant="contained"
                  color="primary"
      ...
      <Box sx={{ backgroundColor: "#FEF9F6", mt: 3 }}>
        ...
        <Grid sx={{p: 5, pl: 40 }} container rowSpacing={1} columnSpacing={{ xs: 1, sm: 2, md: 3 }}>
          <Grid item xs={4}>
